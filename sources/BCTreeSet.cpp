...
// O(1) space
template <typename T>
void BCTreeSet<T>::setRootForTest(SetNode<T>* node) {
    root = node;
}
...
// O(1) space
template <typename T>
void BCTreeSet<T>::fixTree(SetNode<T>* curr, SetNode<T>* parent,
                           SetNode<T>* sibling, bool left) {
...
// O(log(n)) space
template <typename T>
bool BCTreeSet<T>::bstHelp(SetNode<T>* curr, T* prev, bool* lowest) {
    if (!curr) return 1;
